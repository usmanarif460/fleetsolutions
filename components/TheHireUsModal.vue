<template>
  <transition name="fade">
    <article v-if="modalOpen" class="h-screen w-screen fixed top-0 right-0 bg-black/80 z-50">
      <div class=" w-full h-full flex items-center justify-center">
        <dialog class="relative h-4/5 w-4/5 bg-white flex items-center justify-center rounded-md">
          <div class=" flex items-center justify-between w-full px-6">
            <icons-cross-icon class="absolute top-5 right-5" @click="$emit('close')" />
            <div class="px-2 flex-1">
              <div class="flex items-center gap-4">
                <span class="text-purple text-2xl flex items-center self-start">
                  ...
                  <span class="text-purple block">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                      stroke="currentColor" class="size-6 text-purple">
                ...
                <form class="flex flex-col" @submit.prevent="submitStep">
                  ...
                  <input v-model="inputValue" :type="currentType" :id="`input-${currentStep}`"
                  ...
                  <button class="bg-primary max-w-max px-4 py-2 text-3xl text-white font-bold mt-4">
                    OK
                  </button>
                </form>
              </div>
